...
function installRuntime() {
  ...
  return Promise.resolve()
    ...
    .then(() => {
      if (process.platform === "win32") {
        ...
        return pify(fs.remove)(path.join(destination, "runtime"))
          ...
          .then(() => {
            return pify(fs.rename)(
              path.join(destination, "firefox"),
              path.join(destination, "runtime")
            );
          });
        ...
        return new Promise((resolve, reject) => {
          ...
          .then(() => {
            ...
            return pify(fs.remove)(destination).then(() => {
              return pify(fs.copy)(source, destination);
            });
          })
          .then(() => {
            return new Promise((resolve, reject) => {
              const child = spawn("hdiutil", ["detach", mountPoint, "-quiet"], {
                stdio: "inherit",
              });
              child.on("exit", resolve);
              child.on("error", reject);
    ...
    .then(() => {
      ...
      const prefFiles = ["debugger.js", "devtools.js"];
      ...
      switch (process.platform) {
        case "win32": {
          ...
          return pify(fs.copy)(
            ...
            path.join(executableDir, "launcher.bat")
          );
        }
        case "darwin": {
          return pify(fs.copy)(
            path.join(__dirname, "..", "launcher.sh"),
            path.join(executableDir, "launcher.sh")
          ).then(() => {
            // Configure the bundle to run the stub executable.
            const plistFile = path.join(installDir, "Contents", "Info.plist");
            return pify(plist.readFile)(plistFile).then(appPlist => {
