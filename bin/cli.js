...
function usage() {
  console.error(
    [
      ...
      "",
      `More information can be found at ${chalk.cyanBright(
        "https://github.com/Anima-OS/Tabby"
      )}`,
    ].join("\n")
  );
  process.exit(1);
...
function runApp() {
  ...
  const executableDir =
    process.platform === "darwin"
      ? path.join(installDir, "Contents", "MacOS")
      : installDir;
  let executable = path.join(
    "C:\\Program Files\\Quokka",
    `quokka${process.platform === "win32" ? ".exe" : ""}`
  );
  const resourcesDir =
    process.platform === "darwin"
  ...
  const profileDir = fs.mkdtempSync(
    path.join(os.tmpdir(), `${packageJson.name}-profile-`)
  );

  const shellDir = path.join(__dirname, "..", "shell");
  const appDir = fs.existsSync(options.path)
  ...
  readProjectMetadata(appDir, function transformer(appPackageResult) {
    ...
    appPackageResult.pkg.main =
      appPackageResult.pkg.main || appPackageResult.pkg.bin || "index.js";
    ...
    .then(appPackageResult => {
      const mainEntryPoint = path.join(
        appPackageResult.path,
        "..",
        appPackageResult.pkg.main
      );
      ...
      if (options.debug) {
        switch (process.platform) {
          case "win32":
            console.error(
              "The --debug option is not yet supported on Windows."
            );
            process.exit(1);
            break;
          case "darwin":
...
function readProjectMetadata(projectDir, transformer) {
  ...
  return readPkgUp({ cwd: projectDir })
    .then(result => {
